...
10. `happy browser pair` 의 host 핀(127.0.0.1 고정)이 특정 인터페이스
    바인드와 충돌했다. `HAPPY_BROWSER_BRIDGE_HOST=192.168.1.5` 면
    loopback 에는 아무도 없어, pair 가 확장에 죽은 주소를 저장시키고
    실패를 "토큰 불일치"로 오진했다. 5차의 `bridgeProbeHost` 를 재사용해
    "이 머신에서 데몬에 실제로 닿는 주소"(와일드카드/loopback → 127.0.0.1,
    특정 IP → 그 IP)를 핀한다.

9차 — 이미 가진 정보를 버린 곳:

11. `pair` 가 `fetchBrowserStatus` 를 두 번 호출하면서 `hasRecentAuthFailure`
    를 두 번 다 버렸다. 마지막 실패 메시지는 "토큰이나 브리지 포트가
    어긋난 경우"라고 **추측**했는데, 데몬은 어느 쪽인지 이미 알고 있었다.
    거부가 있었다면 확장이 브리지에 닿았다는 뜻이므로 주소는 맞고 토큰이
    다른 것(보통 다른 HAPPY_HOME_DIR 데몬이 포트를 쥔 경우)이고, 거부가
    없었다면 아예 도달하지 못한 것(주소/포트 불일치)이다 — 처방이 정반대다.
    실패 원인 구분이 이 명령의 존재 이유이므로 추측은 그 자체로 결함이다.
    플래그는 대기 구간 동안 sticky 로 누적한다 — 데몬 쪽 창(60초)이
    만료돼 마지막 폴에서 사라져도 그 거부는 여전히 이번 실패의 설명이다.
