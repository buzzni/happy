...
## 2차 리뷰 (코드 결함 없음 판정, 확인 항목 기록)

- **동시 `terminate()` 호출** (idle 워치독과 명시적 close가 경합): 공유 상태 없이 각자
  SIGHUP→확인→SIGKILL을 진행하는데, 같은 프로세스 그룹에 신호가 중복 전달되는 것은
  무해하고 각 호출은 독립적으로 올바른 outcome을 반환한다. in-flight 프로미스 메모로
  단일화할 수도 있으나 코드만 늘고 이득이 없어 하지 않음.
- **데몬 자체가 죽는 경우**: 승급 타이머도 함께 죽지만, 위 커널 SIGHUP 동작이 셸을
  회수하므로 구멍이 아니다(실측).
- **`escaped` outcome은 테스트 불가**: SIGKILL을 견디는 프로세스(D-state 등)를 유닛
  테스트에서 만들 수 없다. 로깅 경로만 존재 확인.
- **`killAll` 반환값 의미**: 기존에도 `kill()`이 절대 throw하지 않아 카운트는 항상
  `sessions.size`였다. 시그니처 변경 후에도 로그 의미 동일(회귀 아님).
...
이 수정은 신규 누수만 막는다. 기존 좀비를 지우는 방법 두 가지:

1. **데몬 재시작(권장)** — 2차 리뷰에서 실측 확인: 데몬 프로세스가 죽으면 커널이 PTY
   마스터를 닫으면서 셸 세션에 SIGHUP을 보내 **모든 좀비가 자동 회수된다**. 좀비가
   지금까지 쌓여 있었던 것 자체가 데몬이 장기 실행 중이었다는 뜻이다. 이 사실 때문에
   tasks.md 이월 항목이던 "데몬 재시작 시 이전 세대 PTY 회수 기능"도 불필요 판명.
2. 데몬을 재시작하고 싶지 않으면 수동 SIGHUP:
