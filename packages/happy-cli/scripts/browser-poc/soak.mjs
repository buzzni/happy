...
for (let minute = 0; minute < warmup + minutes; minute++) {
  ...
  const sample = { minute, atMs: Date.now(), phase: minute < warmup ? 'warmup' : 'measure', completed, containers, driver: debug.drivers, browserPageTargets: { a: targets(env.containers.browserA), b: targets(env.containers.browserB) }, processRssBytes: { browserA: chromiumRss(env.containers.browserA), browserB: chromiumRss(env.containers.browserB) }, journalBytes: journalBytes(), heartbeat: { taskId: task.taskId, stateVersion: retained.stateVersion, updatedAtMs: retained.updatedAtMs }, limits: Object.fromEntries(Object.entries(env.containers).filter(([k]) => ['runtime','browserA','browserB'].includes(k)).map(([k,v]) => [k, docker('inspect','-f','{{.HostConfig.Memory}} {{.HostConfig.NanoCpus}}',v)])) };
...
const maxPageTargets = Math.max(...samples.map(s => Math.max(s.browserPageTargets.a, s.browserPageTargets.b)));
...
const summary = { run, minutes, warmup, completed, crashes, taskLoss, registryStable, result, processRss, maxPageTargets, pass: crashes===0 && taskLoss===0 && registryStable && Object.values(result).every(v=>v.pass) && Object.values(processRss).every(v=>v.pass) };
