...
const barriers = new Map(), faults = new Map(), sessions = new Map()
const key = (run, part) => `${run}\0${part}`
const esc = v => String(v ?? '').replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;').replaceAll("'", '&#39;')
const script = v => JSON.stringify(String(v ?? '')).replaceAll('<', '\\u003c')
const html = (res, body) => send(res, 200, `<!doctype html><meta charset="utf-8"><body>${body}</body>`, 'text/html; charset=utf-8')
...
const record = entry => { const fd = openSync(LEDGER_FILE, 'a', 0o600); try { writeSync(fd, JSON.stringify({ ...entry, atMs: Date.now() }) + '\n'); fsyncSync(fd) } finally { closeSync(fd) } }
...
const body = async req => { let value = ''; for await (const chunk of req) { value += chunk; if (value.length > 65536) throw new Error('body too large') } if (req.headers['content-type']?.includes('application/json')) return JSON.parse(value || '{}'); return Object.fromEntries(new URLSearchParams(value)) }
const auth = req => { const supplied = req.headers['x-harness-token']; const a = Buffer.from(Array.isArray(supplied) ? '' : supplied || ''); const b = Buffer.from(HARNESS_TOKEN); return a.length === b.length && timingSafeEqual(a, b) }
...
const nextPath = raw => { try { const u = new URL(raw, 'http://local'); return u.origin === 'http://local' ? u.pathname + u.search : '/' } catch { return '/' } }
...
http.createServer(async (req, res) => {
 try {
  ...
  if (req.method === 'POST' && u.pathname === '/control/barrier/release') { const { run, key: k, nonce } = await body(req); if (!run || !k || !nonce) return send(res, 400, { error: 'missing fields' }); barriers.set(key(run, k), nonce); record({ kind: 'barrier-release', run, key: k, nonce }); return send(res, 200, { released: true }) }
  if (req.method === 'POST' && u.pathname === '/control/fault') { const { kind, mode, run, delayMs } = await body(req); if (!['risky', 'answer'].includes(kind) || !run || (kind === 'risky' && !['normal', 'hang', 'drop-after-record'].includes(mode))) return send(res, 400, { error: 'invalid fault' }); faults.set(key(run, kind), { mode, delayMs: Number(delayMs ?? mode) || 0 }); return send(res, 200, { ok: true }) }
  if (req.method === 'POST' && u.pathname === '/control/reset') { const { run } = await body(req); if (!run) return send(res, 400, { error: 'missing run' }); for (const map of [barriers, faults]) for (const k of map.keys()) if (k.startsWith(`${run}\0`)) map.delete(k); const kept = existsSync(LEDGER_FILE) ? readFileSync(LEDGER_FILE, 'utf8').split('\n').filter(Boolean).filter(x => JSON.parse(x).run !== run) : []; const fd = openSync(LEDGER_FILE, 'w', 0o600); try { writeSync(fd, kept.length ? kept.join('\n') + '\n' : ''); fsyncSync(fd) } finally { closeSync(fd) } return send(res, 200, { reset: true }) }
...
http.createServer(async (req, res) => {
 try {
  ...
  if (site === 'b') {
   if (p === '/frame-b') return html(res, `<button onclick="fetch('/api/click',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({run:${script(run)},target:'B-frame'})})">Buy</button>`)
  ...
  if (p === '/api/answer' && req.method === 'POST') { const d = await body(req); const correct = barriers.has(key(d.run, d.key)) && d.answer === barriers.get(key(d.run, d.key)); record({ kind: 'answer', run: d.run, key: d.key, correct }); await sleep(faults.get(key(d.run, 'answer'))?.delayMs); return send(res, 200, { correct }) }
  if (p === '/api/click' && req.method === 'POST') { const d = await body(req); record({ kind: 'click', run: d.run, target: d.target }); return send(res, 200, { ok: true }) }
  if (p === '/api/risky' && req.method === 'POST') { const d = await body(req); record({ kind: 'risky', run: d.run, amount: d.amount, requestId: d.requestId }); const mode = faults.get(key(d.run, 'risky'))?.mode; if (mode === 'drop-after-record') return req.socket.destroy(); if (mode === 'hang') return; return send(res, 200, { ok: true }) }
  if (site !== 'a') return send(res, 404, { error: 'not found' })
  if (p === '/login' && req.method === 'POST') { const d = await body(req), sid = randomUUID(); sessions.set(sid, String(d.user || 'user')); res.setHeader('set-cookie', `abp_session=${sid}; HttpOnly; Path=/; SameSite=Lax`); return redirect(res, nextPath(d.next || '/protected')) }
  ...
  if (p === '/protected' || p === '/storage-setup') { const user = sessions.get(cookie(req).abp_session); if (!user) return redirect(res, `/login?next=${encodeURIComponent(p)}`); if (p === '/protected') return html(res, `AUTHENTICATED as ${esc(user)}`); return html(res, `<script>(async()=>{localStorage.setItem('abp_ls','canary');const r=indexedDB.open('abp',1);r.onupgradeneeded=()=>r.result.createObjectStore('kv');r.onsuccess=()=>{const t=r.result.transaction('kv','readwrite');t.objectStore('kv').put('canary','canary');t.oncomplete=()=>document.body.textContent='STORAGE SET'}})()</script>`) }
  ...
  if (p === '/challenge' && req.method === 'POST') { res.setHeader('set-cookie','abp_challenge=ok; HttpOnly; Path=/; SameSite=Lax'); return redirect(res,nextPath((await body(req)).next||'/challenge-protected')) }
  ...
  if (p === '/slow') { await sleep(u.searchParams.get('ms')); return html(res,'SLOW DONE') }
  ...
  send(res, 404, { error: 'not found' })
 } catch { if (!res.headersSent) send(res, 400, { error: 'bad request' }) }
