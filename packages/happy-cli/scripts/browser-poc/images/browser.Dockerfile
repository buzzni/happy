...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends chromium xvfb x11vnc novnc websockify socat python3 tini procps ca-certificates curl && rm -rf /var/lib/apt/lists/* && useradd -u 1000 -m -s /bin/sh browser && mkdir -p /run/abp /home/browser/profile && chown -R browser:browser /run/abp /home/browser
