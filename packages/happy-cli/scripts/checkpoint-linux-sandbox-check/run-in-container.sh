...
bwrap --ro-bind / / --unshare-user --unshare-pid --proc /proc true && echo "bwrap userns OK"
...
const source = 'packages/happy-wire';
const target = 'packages/happy-cli/node_modules/@slopus/happy-wire';
if (fs.realpathSync(source) !== fs.realpathSync(target)) {
    fs.cpSync(`${source}/dist`, `${target}/dist`, { recursive: true });
}
JS
