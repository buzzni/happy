...
export const CHANGE_TITLE_INSTRUCTION = trimIdent(
  `Based on this message, call functions.happy__change_title once to generate a concise chat session title that represents the current task, and a branchSlug: a short English kebab-case slug (2-4 words, lowercase, hyphen-separated, no punctuation) summarizing the same task, for use as a git branch name regardless of the title's language. The title locks after it is first set, so do not call this function again.`
);
