...
describe('observed source fallback (specs/agent-ai-source-routing observation increment)', () => {
    const claude = {
        sessionId: 'happy-session-1',
        occurredAt: 1_788_000_000_000,
        transcriptUuid: 'transcript-uuid',
    ...
    it('reports the observed org-bundle when the daemon confirmed nothing', () => {
        expect(createClaudeUsageEvent({
            ...claude,
            env: { HAPPY_AI_AUTH_SOURCE: 'unknown', HAPPY_AI_AUTH_OBSERVED_SOURCE: 'org-bundle' },
        }).aiAuth).toEqual({ appliedSource: 'org-bundle', connectionVersion: null });
    });

    it('never attaches a connection version to an observation', () => {
        // Only a managed run writes a real version; an observation has none, and a
        // left-over value must not ride along with it.
        expect(createClaudeUsageEvent({
            ...claude,
            env: {
                HAPPY_AI_AUTH_OBSERVED_SOURCE: 'org-bundle',
                HAPPY_AI_AUTH_CONNECTION_VERSION: '5',
            },
        }).aiAuth).toEqual({ appliedSource: 'org-bundle', connectionVersion: null });
    });

    it('lets a confirmed source win over an observation', () => {
        expect(createClaudeUsageEvent({
            ...claude,
            env: { HAPPY_AI_AUTH_SOURCE: 'platform-glm', HAPPY_AI_AUTH_OBSERVED_SOURCE: 'org-bundle' },
        }).aiAuth).toEqual({ appliedSource: 'platform-glm', connectionVersion: null });
    });

    it.each(['', 'personal-subscription', 'ORG-BUNDLE', 'unknown'])(
        'ignores an observed value of %j — the observation channel only ever carries org-bundle',
        (observed) => {
            expect(createClaudeUsageEvent({
                ...claude,
                env: { HAPPY_AI_AUTH_OBSERVED_SOURCE: observed },
            }).aiAuth).toBeUndefined();
        },
    );

    it('produces an event the strict wire schema accepts', () => {
        expect(() => ProviderUsageEventV1Schema.parse(createClaudeUsageEvent({
            ...claude,
            env: { HAPPY_AI_AUTH_OBSERVED_SOURCE: 'org-bundle' },
        }))).not.toThrow();
    });
});
