...
import { requestBrowser, fetchBrowserStatus, BrowserClientError } from './browserClient'

/** Stands in for the daemon control server's /browser/request route. */
function startFakeDaemon(
    handler: (body: any, headers: http.IncomingHttpHeaders) => { status: number; body: any },
) {
    const server = http.createServer((req, res) => {
        let raw = ''
        req.on('data', (chunk) => { raw += chunk })
        req.on('end', () => {
            const { status, body } = handler(JSON.parse(raw || '{}'), req.headers)
...
describe('requestBrowser', () => {
    ...
    it('returns the extension result on success', async () => {
        ...
        const result = await requestBrowser({ port: daemon.port, controlSecret: 'test-secret', method: 'tabs_list', params: { a: 1 } })
    ...
    it('throws NO_EXTENSION_CONNECTED when the daemon reports 503', async () => {
        ...
        await expect(requestBrowser({ port: daemon.port, controlSecret: 'test-secret', method: 'tabs_list' }))
    ...
    it('surfaces an extension-side error code', async () => {
        ...
        await expect(requestBrowser({ port: daemon.port, controlSecret: 'test-secret', method: 'browser_click' }))
    ...
    it('throws DAEMON_UNREACHABLE when nothing is listening', async () => {
        ...
        await daemon.close()
        await expect(requestBrowser({ port: deadPort, controlSecret: 'test-secret', method: 'tabs_list' }))
    ...
    it('authenticates with the given controlSecret', async () => {
        let receivedAuth: string | undefined
        daemon = await startFakeDaemon((body, headers) => {
            receivedAuth = headers.authorization
            return { status: 200, body: { result: {} } }
        })
        await requestBrowser({ port: daemon.port, controlSecret: 'my-secret', method: 'tabs_list' })
...
describe('fetchBrowserStatus', () => {
    ...
    it('authenticates with the given controlSecret', async () => {
        ...
        const server = http.createServer((req, res) => {
            ...
            res.end(JSON.stringify({ connections: [], hasRecentAuthFailure: false }))
        })
        daemon = await new Promise((resolve) => {
            server.listen(0, '127.0.0.1', () => resolve({
                port: (server.address() as AddressInfo).port,
                close: () => new Promise<void>((r) => server.close(() => r())),
            }))
        })
        await fetchBrowserStatus(daemon.port, 'my-secret')
        expect(receivedAuth).toBe('Bearer my-secret')
    })
})
