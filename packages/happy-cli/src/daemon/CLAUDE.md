...
4. `startDaemon()` performs startup:
   ...
   - Heartbeat loop: every 60s by default checks for version updates and prunes dead sessions
...
- Critical timing: heartbeat interval must not fire while dist/ is being rebuilt
