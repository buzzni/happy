...
describe('대상 SHA 테스트 실행 검증', () => {
  it('dispatch input 에서 worktree 요청을 만든다', () => {
    expect(reviewWorktreeRequestFromDispatchInput({ prNumber: 322, headSha: 'a'.repeat(40) }))
      .toEqual({ pullRequest: { number: 322, expectedHeadSha: 'a'.repeat(40) } });
  });

  it('브랜치 이름 같은 값은 받지 않는다', () => {
    expect(reviewWorktreeRequestFromDispatchInput({ prNumber: 322, headSha: 'main' })).toBeNull();
  });
});
