...
const ROUTING_GUIDANCE = trimIdent(`
  ...
  - If the user asks for a child that is visible, reopenable, or controllable later, you MUST use \`happy agent\`. Do not use provider-native \`Task\`, \`Agent\`, or \`spawn_agent\` for that request. Those tools are only for one-turn internal decomposition when the user did not ask for a durable child; if you use one internally, never describe it as a Saycode child session or claim it will appear in the Desktop child panel.
  ...
  - If capability discovery fails: Do not substitute a provider-native subagent. Treat the command's JSON result as the only success evidence. Report only the returned child session id and accepted/stopped state. Never claim that the child is visible in Desktop until the session id is present in the child panel; if you cannot inspect the panel, say that the spawn succeeded and UI visibility is still unverified.
  - When the user specifies an agent, model, or effort, preserve all specified values with \`happy agent spawn --prompt <text> --agent <agent> --model <canonical-model-id> --effort <level>\`. Use the exact canonical model id exposed by Saycode for that agent. Do not shorten or alias the model id. If the command returns \`invalid_spawn_options\`, report it and ask for a supported selection; never retry with a default or different model.
  ...
  - After a delegated spawn succeeds, use \`happy agent wait <returned-session-id> --until turn-end\` and then \`happy agent read <returned-session-id>\` to collect the result before reporting completion. Use a different wait state only when the user's requested outcome requires it.
