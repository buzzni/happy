...
describe('common prompt — child session capability', () => {
    it('moves discovery out of the AX-only base prompt', () => {
        expect(BASE_PROMPT).not.toContain('agent spawn');
        expect(AGENT_ORCHESTRATION_SYSTEM_PROMPT).toContain('happy agent spawn');
    });
    ...
    it('shows how to hand the new conversation its context', () => {
        ...
        expect(AGENT_ORCHESTRATION_SYSTEM_PROMPT).toContain('--file <absolute-path>');
    });
    ...
    it('names the check that tells the assistant whether spawning is available right now', () => {
        ...
        expect(AGENT_ORCHESTRATION_SYSTEM_PROMPT).toContain('happy agent whoami');
    });
});
