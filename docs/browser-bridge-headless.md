...
- **가장 안전:** 공인 인터넷에 그대로 열지 말고, VPN이나 SSH 리버스 터널
  (`ssh -R 41777:127.0.0.1:41777 user@remote`)로 접속을 제한한다. 이 경우
  `HAPPY_BROWSER_BRIDGE_HOST`는 필요 없다 — 확장은 여전히 `127.0.0.1`(터널의
  로컬 끝)로 접속한다.
